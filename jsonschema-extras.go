...
func (me *JsonSchema) generateHandlingScaffold(buf *ustr.Buffer, baseTypeNameIn string, baseTypeNameOut string, ctorcandidates map[string][]string) {
	...
	for tni, tno := range inouts {
		buf.Writeln("\n// Called by `Handle" + baseTypeNameIn + "` after unmarshaling the given `" + tni + "` to populate the given `" + tno + "` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).")
